...
/* --- bugün ---
   Gerçek tarih kullanılır: vade, gecikme ve "bu ay" hesapları ancak böyle
   doğru çalışır. Örnek veri de buna göre üretilir, yani demo her zaman
   güncel görünür. */
...
/** daysAgo: bugünden kaç gün önce. Sabit tarih yerine göreli çapa. */
function addSale(daysAgo, custId, staffId, items, dueDays, payPlan) {
...
(function seedHistory() {
  ...
  for (let back = 11; back >= 1; back--) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      addSale(daysBetween(d, TODAY), pick(cIds), pick(sIds), items, 30, plan);
    }
  }
})();
...
addSale(24, 'c1', 's2', [['p7', 1, 16000], ['p10', 1, 7600], ['p11', 1, 11400]], 30,
        [[6, 10500, 'transfer'], [19, 7000, 'cash']]);

// Ahmad Rahimi — kapanmış geçmiş satış
addSale(97, 'c1', 's1', [['p3', 1, 17500], ['p8', 2, 3400]], 14,
        [[0, 24300, 'card']]);

// Kunduz Electronics — kurumsal, kısmi ödeme
addSale(15, 'c3', 's1', [['p1', 3, 77000], ['p7', 4, 16000]], 30,
        [[2, 140000, 'transfer']]);

// Zahra Hosseini — gecikmiş
addSale(62, 'c2', 's3', [['p4', 1, 90000]], 30,
        [[1, 35000, 'transfer']]);

// Ariana Systems — gecikmiş, büyük bakiye
addSale(50, 'c6', 's1', [['p5', 4, 42000], ['p12', 2, 30000]], 30,
        [[3, 85000, 'transfer']]);

// Fatima Nazari — tamamlandı
addSale(8, 'c4', 's2', [['p13', 1, 30000], ['p14', 2, 3200]], 15,
        [[0, 36400, 'card']]);

// Mohammad Ayubi — ödeme bekliyor (henüz hiç tahsilat yok)
addSale(5, 'c5', 's3', [['p2', 1, 56000]], 21, []);

// Maryam Sadat — tamamlandı
addSale(3, 'c7', 's2', [['p7', 2, 16000], ['p9', 1, 2000]], 7,
        [[0, 34000, 'cash']]);

// Omar Stanikzai — kısmi
addSale(2, 'c8', 's3', [['p6', 1, 66500]], 30,
        [[0, 28000, 'card']]);

// Salma Karimi — tamamlandı
addSale(1, 'c9', 's2', [['p10', 1, 7600], ['p8', 1, 3400]], 7,
        [[0, 11000, 'cash']]);

// Bilal Ahmadzai — ödeme bekliyor
addSale(1, 'c10', 's1', [['p11', 2, 11400]], 14, []);
